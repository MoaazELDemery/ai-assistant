...
// Check if message is asking about exchange rates
function isExchangeRateQuery(message: string): { isQuery: boolean; from?: string; to?: string } {
    // Clean message - remove dashes between letters (like S-A-R)
...
// Generate local exchange rate response
    ...
    static async sendMessage(content: string, sessionId: string, locale: string): Promise<any> {
        try {
            ...
            if (typeof jsonString === 'object') {
                ...
                // Fallback to local if n8n couldn't get exchange rate
                if (parsed.message &&
